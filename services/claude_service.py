...
class ClaudeService:
    ...
    def _clean_json_string(self, json_str: str) -> str:
        ...
        # This is a more aggressive fix for quote issues
        lines = cleaned.split('\n')
        fixed_lines = []

        for line in lines:
            # Skip lines that are proper JSON structure
            if re.match(r'^\s*[{\[\]}],?\s*$', line):
                fixed_lines.append(line)
                continue

            # Fix lines with potential quote issues
        ...
        # Final cleanup - remove any trailing commas before closing
