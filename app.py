...
# ======= SCHEMA-BASED EXTRACTION ENDPOINTS =======

@app.route('/api/schema-extraction/<doc_id>', methods=['POST'])
def api_schema_extraction(doc_id):
    """Schema-based extraction with vision validation"""
    doc = storage.get_document(doc_id)
    if not doc:
        return jsonify({'success': False, 'error': 'Document not found'}), 404
    ...
    try:
        ...
        use_visual_validation = data.get('use_visual_validation', True)
        ...
        schema_extractor = SchemaTextExtractor(api_key)

        # Process enhanced workflow with visual validation
        result = schema_extractor.enhanced_extraction_workflow(
            doc.filepath, schema, page_num, use_visual_validation
        )

        if result['success']:
            # Save result
            doc.set_step_result('schema_extraction', result)
            storage.update_document(doc)

            # Get correction details for UI display
            correction_details = result.get('detailed_results', {}).get('visual_validation_summary', {})
            ...
            return jsonify({
                ...
                'visual_validation_applied': result['workflow_summary'].get('visual_validation_applied', False),
                'corrections_applied': result['workflow_summary'].get('corrections_applied', 0),
                'correction_details': correction_details,
                'correction_history': correction_details.get('correction_history', [])
            })
        else:
            return jsonify({
                'success': False,
                'error': result.get('error', 'Schema extraction failed')
            }), 500
...
@app.route('/schema-extraction')
def schema_extraction_page():
    """Schema-based extraction interface"""
    recent_docs = storage.get_recent_documents(limit=10)
    return render_template('schema_extraction.html', recent_docs=recent_docs)
